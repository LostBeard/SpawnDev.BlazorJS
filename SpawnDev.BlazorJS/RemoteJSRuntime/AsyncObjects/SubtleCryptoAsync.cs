...
namespace SpawnDev.BlazorJS.RemoteJSRuntime.AsyncObjects
{
    ...
    public class SubtleCryptoAsync
    {
        ...
        /// </summary>
        public SubtleCryptoAsync(IJSRuntime jsr)
        {
            JSR = jsr;
        }
        /// <summary>
        ...
        public Task<ArrayBufferAsync> DeriveBits(KeyDeriveParams algorithm, CryptoKeyAsync baseKey, int length) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.deriveBits", algorithm, baseKey, length);
        /// <summary>
        ...
        public Task<CryptoKeyAsync> DeriveKey(KeyDeriveParams algorithm, CryptoKeyAsync baseKey, HmacKeyGenParams derivedKeyAlgorithm, bool extractable, string[] keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.deriveKey", algorithm, baseKey, derivedKeyAlgorithm, extractable, keyUsages);
        /// <summary>
        /// The deriveKey() method of the SubtleCrypto interface can be used to derive a secret key from a master key.<br/>
        /// It takes as arguments some initial key material, the derivation algorithm to use, and the desired properties for the key to derive. It returns a Promise which will be fulfilled with a CryptoKey object representing the new key.<br/>
        /// It's worth noting that the three key derivation algorithms you can use have quite different characteristics and are appropriate in quite different situations. See Supported algorithms for some more detail on this.<br/>
        ...
        /// </param>
        /// <returns>A Promise that fulfills with a CryptoKey.</returns>
        public Task<CryptoKeyAsync> DeriveKey(KeyDeriveParams algorithm, CryptoKeyAsync baseKey, AesKeyGenParams derivedKeyAlgorithm, bool extractable, string[] keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.deriveKey", algorithm, baseKey, derivedKeyAlgorithm, extractable, keyUsages);
        /// <summary>
        /// The deriveKey() method of the SubtleCrypto interface can be used to derive a secret key from a master key.<br/>
        /// It takes as arguments some initial key material, the derivation algorithm to use, and the desired properties for the key to derive. It returns a Promise which will be fulfilled with a CryptoKey object representing the new key.<br/>
        /// It's worth noting that the three key derivation algorithms you can use have quite different characteristics and are appropriate in quite different situations. See Supported algorithms for some more detail on this.<br/>
        ...
        /// </param>
        /// <returns>A Promise that fulfills with a CryptoKey.</returns>
        public Task<CryptoKeyAsync> DeriveKey(KeyDeriveParams algorithm, CryptoKeyAsync baseKey, HkdfParams derivedKeyAlgorithm, bool extractable, string[] keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.deriveKey", algorithm, baseKey, derivedKeyAlgorithm, extractable, keyUsages);
        /// <summary>
        /// The deriveKey() method of the SubtleCrypto interface can be used to derive a secret key from a master key.<br/>
        /// It takes as arguments some initial key material, the derivation algorithm to use, and the desired properties for the key to derive. It returns a Promise which will be fulfilled with a CryptoKey object representing the new key.<br/>
        /// It's worth noting that the three key derivation algorithms you can use have quite different characteristics and are appropriate in quite different situations. See Supported algorithms for some more detail on this.<br/>
        ...
        /// </param>
        /// <returns>A Promise that fulfills with a CryptoKey.</returns>
        public Task<CryptoKeyAsync> DeriveKey(KeyDeriveParams algorithm, CryptoKeyAsync baseKey, Pbkdf2Params derivedKeyAlgorithm, bool extractable, string[] keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.deriveKey", algorithm, baseKey, derivedKeyAlgorithm, extractable, keyUsages);
        /// <summary>
        ...
        public Task<ArrayBufferAsync> Digest(string algorithm, byte[] data) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.digest", algorithm, data);
        /// <summary>
        ...
        /// </returns>
        public Task<T> ExportKey<T>(string format, CryptoKeyAsync key) => JSR.CallAsync<T>("crypto.subtle.exportKey", format, key);
        /// <summary>
        ...
        public Task<JSObjectAsync> ExportKeyJwk(CryptoKeyAsync key) => JSR.CallAsync<JSObjectAsync>("crypto.subtle.exportKey", "jwk", key);
        /// <summary>
        ...
        public Task<ArrayBufferAsync> ExportKeyPkcs8(CryptoKeyAsync key) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.exportKey", "pkcs8", key);
        /// <summary>
        ...
        /// <param name="key">the CryptoKey to export</param>
        /// <returns>A Promise that fulfills with an ArrayBuffer containing the key</returns>
        public Task<ArrayBufferAsync> ExportKeyRaw(CryptoKeyAsync key) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.exportKey", "raw", key);
        /// <summary>
        ...
        /// <param name="key">the CryptoKey to export</param>
        /// <returns>A Promise that fulfills with an ArrayBuffer containing the key</returns>
        public Task<ArrayBufferAsync> ExportKeySpki(CryptoKeyAsync key) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.exportKey", "spki", key);
        /// <summary>
        ...
        public Task<T> GenerateKey<T>(RsaHashedKeyGenParams algorithm, bool extractable, IEnumerable<string> keyUsages) where T : CryptoKeyBaseAsync => JSR.CallAsync<T>("crypto.subtle.generateKey", algorithm, extractable, keyUsages);
        /// <summary>
        /// Use the generateKey() method of the SubtleCrypto interface to generate a new key (for symmetric algorithms) or key pair (for public-key algorithms).
        /// </summary>
        /// <param name="algorithm">
        ...
        /// <param name="keyUsages">An Array indicating what can be done with the key. Possible array values are: encrypt, decrypt, sign, verify, deriveKey, deriveBits, wrapKey, or unwrapKey.</param>
        /// <returns>A Promise that fulfills with a CryptoKey (for symmetric algorithms) or a CryptoKeyPair (for public-key algorithms).</returns>
        public Task<T> GenerateKey<T>(EcKeyGenParams algorithm, bool extractable, IEnumerable<string> keyUsages) where T : CryptoKeyBaseAsync => JSR.CallAsync<T>("crypto.subtle.generateKey", algorithm, extractable, keyUsages);
        /// <summary>
        /// Use the generateKey() method of the SubtleCrypto interface to generate a new key (for symmetric algorithms) or key pair (for public-key algorithms).
        /// </summary>
        /// <param name="algorithm">
        ...
        /// <param name="keyUsages">An Array indicating what can be done with the key. Possible array values are: encrypt, decrypt, sign, verify, deriveKey, deriveBits, wrapKey, or unwrapKey.</param>
        /// <returns>A Promise that fulfills with a CryptoKey (for symmetric algorithms) or a CryptoKeyPair (for public-key algorithms).</returns>
        public Task<T> GenerateKey<T>(HmacKeyGenParams algorithm, bool extractable, IEnumerable<string> keyUsages) where T : CryptoKeyBaseAsync => JSR.CallAsync<T>("crypto.subtle.generateKey", algorithm, extractable, keyUsages);
        /// <summary>
        /// Use the generateKey() method of the SubtleCrypto interface to generate a new key (for symmetric algorithms) or key pair (for public-key algorithms).
        /// </summary>
        /// <param name="algorithm">
        ...
        /// <param name="keyUsages">An Array indicating what can be done with the key. Possible array values are: encrypt, decrypt, sign, verify, deriveKey, deriveBits, wrapKey, or unwrapKey.</param>
        /// <returns>A Promise that fulfills with a CryptoKey (for symmetric algorithms) or a CryptoKeyPair (for public-key algorithms).</returns>
        public Task<T> GenerateKey<T>(AesKeyGenParams algorithm, bool extractable, IEnumerable<string> keyUsages) where T : CryptoKeyBaseAsync => JSR.CallAsync<T>("crypto.subtle.generateKey", algorithm, extractable, keyUsages);
        /// <summary>
        ...
        public Task<CryptoKeyAsync> ImportKey(string format, byte[] keyData, CryptoImportParams algorithm, bool extractable, IEnumerable<string> keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.importKey", format, keyData, algorithm, extractable, keyUsages);
        /// <summary>
        /// The importKey() method of the SubtleCrypto interface imports a key: that is, it takes as input a key in an external, portable format and gives you a CryptoKey object that you can use in the Web Crypto API.
        /// </summary>
        /// <param name="format">
        ...
        /// <param name="keyUsages">An Array indicating what can be done with the key. Possible array values are: encrypt, decrypt, sign, verify, deriveKey, deriveBits, wrapKey, or unwrapKey.</param>
        /// <returns>A Promise that fulfills with the imported key as a CryptoKey object.</returns>
        public Task<CryptoKeyAsync> ImportKey(string format, byte[] keyData, string algorithm, bool extractable, IEnumerable<string> keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.importKey", format, keyData, algorithm, extractable, keyUsages);
        /// <summary>
        ...
        public Task<ArrayBufferAsync> Sign(CryptoSignParams algorithm, CryptoKeyAsync key, byte[] data) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.sign", algorithm, key, data);
        /// <summary>
        /// The sign() method of the SubtleCrypto interface generates a digital signature.<br/>
        /// It takes as its arguments a key to sign with, some algorithm-specific parameters, and the data to sign. It returns a Promise which will be fulfilled with the signature.
        /// </summary>
        ...
        /// <param name="data">An ArrayBuffer, a TypedArray or a DataView object containing the data to be signed.</param>
        /// <returns>A Promise that fulfills with an ArrayBuffer containing the signature.</returns>
        public Task<ArrayBufferAsync> Sign(string algorithm, CryptoKeyAsync key, byte[] data) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.sign", algorithm, key, data);
        /// <summary>
        ...
        public Task<CryptoKeyAsync> UnwrapKey(string format, ArrayBufferAsync wrappedKey, CryptoKeyAsync unwrappingKey, EncryptParams unwrapAlgo, CryptoImportParams unwrappedKeyAlgo, bool extractable, IEnumerable<string> keyUsages) => JSR.CallAsync<CryptoKeyAsync>("crypto.subtle.unwrapKey", format, wrappedKey, unwrappingKey, unwrapAlgo, unwrappedKeyAlgo, extractable, keyUsages);
        /// <summary>
        ...
        public Task<bool> Verify(CryptoSignParams algorithm, CryptoKeyAsync key, ArrayBufferAsync signature, byte[] data) => JSR.CallAsync<bool>("crypto.subtle.verify", algorithm, key, signature, data);
        /// <summary>
        /// The verify() method of the SubtleCrypto interface verifies a digital signature.<br/>
        /// It takes as its arguments a key to verify the signature with, some algorithm-specific parameters, the signature, and the original signed data.It returns a Promise which will be fulfilled with a boolean value indicating whether the signature is valid.
        /// </summary>
        ...
        public Task<ArrayBufferAsync> WrapKey(string format, CryptoKeyAsync key, CryptoKeyAsync wrappingKey, EncryptParams wrapAlgo) => JSR.CallAsync<ArrayBufferAsync>("crypto.subtle.wrapKey", format, key, wrappingKey, wrapAlgo);
    }
}
